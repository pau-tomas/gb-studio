...
test("should treat variables as length=3 in character limits", () => {
  expect(trimlines("$L0$Hello\nWorld", 5)).toBe("$L0$He\nWorld");
  expect(trimlines("He$L0$\nWorld", 5)).toBe("He$L0$\nWorld");
  expect(trimlines("Hello$L0$\nWorld", 5)).toBe("Hello\nWorld");
});

test("should treat variable characters as length=1 in character limits", () => {
  expect(trimlines("#L0#Hello\nWorld", 5)).toBe("#L0#Hell\nWorld");
  expect(trimlines("Hello#L0#\nWorld", 5)).toBe("Hello\nWorld");
  expect(trimlines("Hell#L0#\nWorld", 5)).toBe("Hell#L0#\nWorld");
});

test("should treat variables as length=3 in total limits", () => {
  expect(trimlines("$L0$Hello\nWorld", 5, 1)).toBe("$L0$He");
  expect(trimlines("He$L0$\nWorld", 5, 1)).toBe("He$L0$");
  expect(trimlines("Hello$L0$\nWorld", 5, 1)).toBe("Hello");
});

test("should treat variable characters as length=1 in total limits", () => {
  expect(trimlines("HelloWorld", 5, 1)).toBe("Hello");
